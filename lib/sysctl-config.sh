...
# however, helps on small attacks
...
# increase open files limit for "root"
LIMITS_SET=$(grep 'root' /etc/security/limits.conf | grep nofile | grep -v '^#')
if [ "${LIMITS_SET}" ]
 then
	echo "root limits for nofiles already set to"
	grep 'root' /etc/security/limits.conf | grep nofil | grep -v '^#'
 else
	echo "Setting limits for user root in /etc/security/limits.conf"
